...
import { getWarehouseProductDetail, signedMovementQuantity } from "@/lib/warehouse/queries";
import {
  formatDate,
  formatDecimal,
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
              ...
              <tbody className="divide-y divide-border">
                {product.warehouseMovements.map((movement) => (
                  <tr key={movement.id}>
                    ...
                    <td className="px-4 py-3">{formatDecimal(signedMovementQuantity(movement))} {movement.unit}</td>
                    <td className="px-4 py-3">{movement.lot ?? "-"}</td>
                    <td className="px-4 py-3">{movement.note ?? movement.adjustment?.reason ?? "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Card>
          ...
          <CardContent>
            <form action={createWarehouseAdjustmentAction} className="space-y-4">
              <input name="productMaterialId" type="hidden" value={product.id} />
              <label className="block text-sm font-medium">
                Data
                <input className="focus-ring mt-2 h-10 w-full rounded-[8px] border border-input bg-background px-3" name="movedOn" required type="date" />
              </label>
              <label className="block text-sm font-medium">
                Quantita (+ carico, - scarico)
                <input className="focus-ring mt-2 h-10 w-full rounded-[8px] border border-input bg-background px-3" inputMode="decimal" name="quantity" required />
              </label>
              <label className="block text-sm font-medium">
                Unita
                <input className="focus-ring mt-2 h-10 w-full rounded-[8px] border border-input bg-background px-3" name="unit" defaultValue={product.unit} required />
              </label>
              <label className="block text-sm font-medium">
                Causale obbligatoria
                <textarea className="focus-ring mt-2 min-h-24 w-full rounded-[8px] border border-input bg-background px-3 py-2" name="reason" required />
              </label>
